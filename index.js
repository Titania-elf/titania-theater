...
// part1 end
// Part 2/2 Start (v3.5 修复版：稳健数据读取)
...
function showDebugInfo() {
    ...
    <div class="t-box" id="t-debug-view" style="height:90vh;">
        <div class="t-header"><span class="t-title-main" style="font-size:1.2em;">👁️ 提示词审查</span><span class="t-close" id="t-debug-close">&times;</span></div>
        <div class="t-body" style="padding:0;">
            <pre class="t-code-block">${JSON.stringify(debugData, null, 2)}</pre>
        </div>
        <div style="padding:10px; border-top:1px solid #444;"><button id="t-debug-back" class="t-btn primary" style="width:100%;">返回主窗口</button></div>
    </div>`;
...
async function handleGenerate() {
    // 1. 严格从 LocalStorage 读取配置，不依赖 DOM
    const cfg = JSON.parse(localStorage.getItem(STORAGE_KEY_CFG) || '{}');
    ...
    try {
        // 2. 组装 Prompt (包含 UserBio 和 WorldInfo)
        ...
        const res = await fetch(endpoint, {
            ...
            body: JSON.stringify({ 
                model: cfg.model || "gpt-3.5-turbo", // 确保有默认值
