// 【Echo Theater v3.6 - Part 1/3】
// 包含：20个默认剧本库、悬浮球、数据存取
...
// mode: 'echo' (回声/上下文相关), 'parallel' (平行世界/IF线), 'all' (通用)
...
$(document).ready(function() {
    console.log("Titania Echo v3.6: Loaded.");
    loadScripts(); 
...
// 【Echo Theater v3.6 - Part 2/3】
// 包含：Context获取(宏+蓝灯)、主界面、模式开关逻辑

// 上下文获取 (宏解析 + 蓝灯世界书 + 用户Persona修正)
function getContextData() {
    ...
    try {
        ...
        data.userDesc = ctx.substituteParams("{{persona}}") || "";     // 用户设定
        data.persona  = ctx.substituteParams("{{description}}") || ""; // 角色设定
    } catch (e) {
        console.error("Titania: 宏解析失败", e);
    }

    // 2. 获取 Character Book 蓝灯常驻条目
    if (ctx.characterId) {
        ...
        if (charBook && Array.isArray(charBook.entries)) {
            const constantEntries = charBook.entries.filter(entry => !entry.disable && entry.constant);
    ...
    if (Array.isArray(globalWI) && globalWI.length > 0) {
        ...
        const activeEntries = globalWI.filter(book => {
...
function refreshScriptList(isEchoMode) {
    ...
    // 筛选：user自定义 || (mode === current || mode === all)
...
function openMainWindow() {
    ...
    const style = `
    <style>
        .t-mode-box { display:flex; align-items:center; justify-content:flex-end; gap:10px; margin-bottom:10px; font-size:0.9em; color:#ccc; }
        .t-switch { position: relative; display: inline-block; width: 40px; height: 20px; }
        .t-switch input { opacity: 0; width: 0; height: 0; }
        .t-slider { position: absolute; cursor: pointer; top: 0; left: 0; right: 0; bottom: 0; background-color: #4a4a4a; transition: .4s; border-radius: 34px; }
        .t-slider:before { position: absolute; content: ""; height: 14px; width: 14px; left: 3px; bottom: 3px; background-color: white; transition: .4s; border-radius: 50%; }
        input:checked + .t-slider { background-color: #bfa15f; }
        input:checked + .t-slider:before { transform: translateX(20px); }
        .t-mode-label { font-weight:bold; transition:color 0.3s; }
        .mode-active { color: #bfa15f; }
    </style>`;
    ...
    <div id="t-overlay" class="t-overlay">
        <div class="t-box" id="t-main-view">
            ...
            <div class="t-body">
                <div style="text-align:center; color:#888; font-size:0.9em; margin-bottom:5px;">✨ 当前主演: ${ctx.charName}</div>
                
                <!-- 模式切换开关 -->
                <div class="t-mode-box">
                    <span id="t-mode-text-p" class="t-mode-label mode-active">平行世界</span>
                    <label class="t-switch">
                        <input type="checkbox" id="t-mode-toggle">
                        <span class="t-slider"></span>
                    </label>
                    <span id="t-mode-text-e" class="t-mode-label">回声模式</span>
                </div>

                <div class="t-controls">
                    <select id="t-sel-script" class="t-select"></select> <!-- JS填充 -->
    ...
    $("#t-mode-toggle").on("change", function() {
        ...
        if(isEcho) {
            $("#t-mode-text-e").addClass("mode-active");
            $("#t-mode-text-p").removeClass("mode-active");
        } else {
            $("#t-mode-text-p").addClass("mode-active");
            $("#t-mode-text-e").removeClass("mode-active");
        }
...
// 【Echo Theater v3.6 - Part 3/3】
// 包含：API生成、历史记录读取、设置(含HistoryLimit)、收藏夹
...
function getChatHistory(limit) {
    ...
    return recent.map(msg => {
        ...
        let cleanContent = msg.message.replace(/<[^>]*>?/gm, ''); 
...
function showDebugInfo() {
    ...
    const debugHtml = `
    <div class="t-box" id="t-debug-view" style="height:95vh; display:flex; flex-direction:column;">
        <div class="t-header">
            <span class="t-title-main">👁️ Prompt 审查</span>
            <span class="t-close" id="t-debug-close">&times;</span>
        </div>
        <div class="t-body" style="padding:10px; overflow-y:auto; flex-grow:1; font-family:monospace; font-size:12px;">
            <div style="margin-bottom:10px; padding:5px; background:#222; border:1px solid #444;">
                <strong style="color:#bfa15f;">[Configuration]</strong><br>
                Model: ${cfg.model || "Default"}<br>
                Mode: ${isEchoMode ? "Echo (History ON)" : "Parallel (History OFF)"}
            </div>

            <div style="margin-bottom:10px;">
                <strong style="color:#ff6b6b;">[System Message]</strong>
                <pre style="white-space:pre-wrap; color:#aaa; margin:5px 0; background:#111; padding:5px;">${sysPrompt}</pre>
            </div>

            <div>
                <strong style="color:#90cdf4;">[User Message (Final Context)]</strong>
                <pre style="white-space:pre-wrap; color:#ddd; margin:5px 0; background:#111; padding:5px; border-left:3px solid #90cdf4;">${userPrompt}</pre>
            </div>
        </div>
...
function openSettingsWindow() {
    ...
    <div class="t-box" id="t-settings-view">
        ...
        <div class="t-body">
            ...
            <h4 style="margin:15px 0 5px 0; border-bottom:1px solid #444; padding-bottom:5px;">🧬 回声模式设置</h4>
            <div><label>读取历史记录条数 (默认10):</label><input id="cfg-history" type="number" class="t-input" value="${cfg.history_limit || 10}"></div>

            <div style="display:flex; justify-content:space-between; align-items:center; margin-top:15px; border-bottom:1px solid #444; padding-bottom:5px;">
